...
ROSTER_COLUMNS = {
    ...
    "morgan_stanley": {"team": ["Team Name"], "team_url": ["Team Page URL"],
...
def load_crm(domains: dict[str, tuple[str, ...]],
    ...
    frame = pd.DataFrame([{
        ...
        "city": n["city"],
        "state": n["state"],
        "location_street": str(addr(r, "line1") or "").strip(),
        "location_zip": str(addr(r, "postalCode") or "").strip(),
        "location_lat": str(addr(r, "latitude") or "").strip(),
        "location_lon": str(addr(r, "longitude") or "").strip(),
...
def load_rosters() -> pd.DataFrame:
    ...
    for slug, meta in FIRMS.items():
        ...
        rec = pd.DataFrame({
            ...
            "city": city,
            "state": state,
            "location_street": pick("address1", "street_line_1", "office_street",
                                    "office_address", "address_line1", "address",
                                    field="location_street").str.strip(),
            "location_zip": pick("postal", "postal_code", "zipcode", "zip",
                                 "faZipCode", field="location_zip").str.strip(),
            "location_lat": pick("lat", "latitude", "GeoLat",
                                 field="location_lat").str.strip(),
            "location_lon": pick("lon", "longitude", "GeoLon",
                                 field="location_lon").str.strip(),
...
def load_people(limit: int | None = None) -> pd.DataFrame:
    ...
    keep = ["name", "title", "team", "team_url", "profile_url", "linkedin", "email",
            ...
            "location_street", "location_zip", "location_lat", "location_lon",
